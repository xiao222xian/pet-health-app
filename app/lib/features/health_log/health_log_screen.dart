...
class _HealthLogScreenState extends State<HealthLogScreen> {
  ...
  void _showDetailSheet(HealthLog log) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _HealthLogDetailSheet(
        petName: _petName ?? '宝贝',
        log: log,
        onEdit: () {
  ...
  void _showCalendarSheet() {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _HealthCalendarSheet(
        logsByDate: _logsByDate,
        onSelectLog: (log) => _showDetailSheet(log),
      ),
    );
  }
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                ...
                if (_logs.isNotEmpty) ...[
                  ...
                  ..._logs
                      .where((l) =>
                          _dateKey(l.logDate) != _dateKey(DateTime.now()))
  ...
  Widget _buildNoPet() => Center(
        ...
        const Text('先去添加你的宠物吧',
            style: TextStyle(
                fontSize: 17,
                fontWeight: FontWeight.w700,
                color: AppTheme.deepBlue)),
        const SizedBox(height: 8),
        const Text('添加宠物后即可记录健康日志',
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (log != null) ...[
            ...
            Wrap(spacing: 8, children: [
              if (log.waterMl != null)
                _statBadge('饮水', _waterStr(log.waterMl!)),
              _statBadge(
                  '大便',
                  log.stoolStatus != null
                      ? _stoolStr(log.stoolStatus!)
                      : '未记录'),
              if (log.foodType != null)
                _statBadge('饮食', _foodStr(log.foodType!)),
    ...
    return GestureDetector(
        ...
        child: Container(
          ...
          child:
            ...
            Row(
              ...
              children: days.map((d) {
                ...
                return Column(children: [
                  Text(isToday ? '今' : wds[d.weekday - 1],
                      style: TextStyle(
                          fontSize: 11,
                          fontWeight:
                              isToday ? FontWeight.w700 : FontWeight.w400,
                          color:
                              isToday ? AppTheme.primary : AppTheme.textHint)),
            ...
            Row(children: [
              _legendDot(AppTheme.danger, '很差'),
              const SizedBox(width: 10),
              _legendDot(AppTheme.warning, '欠佳'),
              const SizedBox(width: 10),
              _legendDot(const Color(0xFF26A69A), '正常'),
              const SizedBox(width: 10),
          ...
          Expanded(
              child: Column(
                ...
                Row(children: [
                  ...
                  Text(_weekdayStr(log.logDate.weekday),
                      style: const TextStyle(
                ...
                Wrap(spacing: 6, runSpacing: 4, children: [
                  if (log.appetiteLevel != null)
                    _miniChip(_energyStr(log.appetiteLevel!), statusColor),
                  if (log.waterMl != null)
                    _miniChip(
                        '水: ${_waterStr(log.waterMl!)}', AppTheme.primary),
                  if (log.stoolStatus != null)
                    _miniChip(
                        _stoolStr(log.stoolStatus!), AppTheme.textSecondary),
                  if (log.foodType != null)
                    _miniChip(_foodStr(log.foodType!), AppTheme.primaryLight),
                  if (log.weightKg != null)
                    _miniChip('${log.weightKg}kg', AppTheme.textSecondary),
                ]),
  ...
  Widget _buildWeightChart() {
    ...
    final points = _logs.where((l) => l.weightKg != null).toList()
...
class _EntrySheetState extends State<_EntrySheet> {
  ...
  Future<void> _save() async {
    ...
    final logDate =
        widget.existing?.logDate.toIso8601String().substring(0, 10) ??
      ...
      children: options.map((o) {
        ...
        return Expanded(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: isSelected
                  ? (soft ? AppTheme.primarySoft : AppTheme.primary)
                  : AppTheme.background,
              borderRadius: BorderRadius.circular(14),
              border: Border.all(
...
class _HealthLogDetailSheet extends StatelessWidget {
  final String petName;
  final HealthLog log;
  final VoidCallback onEdit;

  const _HealthLogDetailSheet({
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              '$petName · ${log.logDate.month}月${log.logDate.day}日',
              style: const TextStyle(
                fontSize: 22,
                fontWeight: FontWeight.w800,
                color: AppTheme.deepBlue,
            ...
            Wrap(
              ...
              children: [
                _detailChip(
                    _HealthLogScreenState._energyStr(log.appetiteLevel ?? 3),
                    AppTheme.primary),
                if (log.waterMl != null)
                  _detailChip(
                      '饮水 ${_HealthLogScreenState._waterStr(log.waterMl!)}',
                      const Color(0xFF26A69A)),
                if (log.stoolStatus != null)
                  _detailChip(_HealthLogScreenState._stoolStr(log.stoolStatus!),
                      AppTheme.textSecondary),
                if (log.foodType != null)
                  _detailChip(
                      '饮食 ${_HealthLogScreenState._foodStr(log.foodType!)}',
                      AppTheme.primaryLight),
                if (log.weightKg != null)
                  _detailChip('体重 ${log.weightKg}kg', AppTheme.warning),
              ],
            ),
            if (log.notes != null && log.notes!.isNotEmpty) ...[
              const SizedBox(height: 16),
...
class _HealthCalendarSheet extends StatefulWidget {
  final Map<String, HealthLog> logsByDate;
  final ValueChanged<HealthLog> onSelectLog;

  const _HealthCalendarSheet({
    required this.logsByDate,
    required this.onSelectLog,
  });

  @override
  State<_HealthCalendarSheet> createState() => _HealthCalendarSheetState();
}

class _HealthCalendarSheetState extends State<_HealthCalendarSheet> {
