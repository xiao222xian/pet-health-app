...
class _TimelineScreenState extends State<TimelineScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _pulseCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 1800))
      ..repeat(reverse: true);
    _pathCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 2000));
    _loadEvents();
  }
  ...
  Future<void> _loadEvents() async {
    if (!mounted) return;
    setState(() { _loading = true; _visible = []; _pathAnims = []; _pathCtrl.reset(); });
    ...
    final data = await SupabaseService.client.from('timeline_events').select()
        .eq('pet_id', _petId!).order('event_date', ascending: false);
    if (!mounted) return;
    final events = (data as List).map((e) => TimelineEvent.fromJson(e as Map<String, dynamic>)).toList();
    _allEvents = events;
  ...
  void _buildPathAnimations(int count) {
    _pathAnims = List.generate(count, (i) {
      ...
      return CurvedAnimation(parent: _pathCtrl, curve: Interval(start, end, curve: Curves.easeOut));
    });
  }

  Future<void> _deleteEvent(TimelineEvent event) async {
    await SupabaseService.client.from('timeline_events').delete().eq('id', event.id);
    ...
    showCupertinoModalPopup(context: context, builder: (_) => CupertinoActionSheet(
      ...
      actions: [
        CupertinoActionSheetAction(
          ...
          onPressed: () { Navigator.of(context).pop(); _deleteEvent(event); },
          child: const Text('删除')),
      ],
            ...
            trailing: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  gradient: _petId != null ? AppTheme.primaryGradient : null,
                  color: _petId == null ? AppTheme.textHint : null,
                  borderRadius: BorderRadius.circular(10),
                  boxShadow: _petId != null ? AppTheme.cardShadowStrong : null),
  ...
  Widget _buildFilterChips() => SizedBox(
    ...
    child: ListView(
      ...
      children: _filters.map((f) {
        ...
        final color = f.$1 == 'all' ? AppTheme.primary : _typeConfig(f.$1).color;
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: sel ? [BoxShadow(color: color.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 2))] : null,
            ),
  ...
  Widget _buildRow(int index) {
    ...
    // Year header
    Widget? yearHeader;
    if (index == 0 || _filtered[index].eventDate.year != _filtered[index - 1].eventDate.year) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      if (!isLast)
        LayoutBuilder(builder: (ctx, constraints) {
          final anim = index < _pathAnims.length ? _pathAnims[index] : const AlwaysStoppedAnimation(0.0);
    ...
    child: Row(children: [
      Container(
        ...
        child: Text('$year', style: const TextStyle(
          color: Colors.white, fontWeight: FontWeight.w800, fontSize: 13, letterSpacing: 0.5))),
      Expanded(child: Container(height: 1.5, margin: const EdgeInsets.only(left: 10),
        decoration: BoxDecoration(
  ...
  Widget _buildCardRow(int index, TimelineEvent event, _TypeConfig cfg,
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: isEven ? [
          SizedBox(width: dotW, child: Padding(padding: const EdgeInsets.only(top: 8),
            child: Center(child: dot))),
          Expanded(child: Padding(padding: const EdgeInsets.only(right: 4), child: card)),
        ] : [
          Expanded(child: Padding(padding: const EdgeInsets.only(left: 4), child: card)),
          SizedBox(width: dotW, child: Padding(padding: const EdgeInsets.only(top: 8),
            child: Center(child: dot))),
        ],
      ),
      ...
      return AnimatedBuilder(
        ...
        builder: (_, __) {
          ...
          return Stack(alignment: Alignment.center, children: [
            Container(
              width: 50 + p * 8, height: 50 + p * 8,
              decoration: BoxDecoration(
                color: cfg.color.withOpacity(0.12 + p * 0.06), shape: BoxShape.circle)),
            _dot(cfg, 40),
          ]);
        },
      );
  ...
  Widget _dot(_TypeConfig cfg, double size) => Container(
    ...
    decoration: BoxDecoration(
      ...
      boxShadow: [BoxShadow(color: cfg.color.withOpacity(0.45), blurRadius: 12, offset: const Offset(0, 4))]),
  ...
  Widget _buildCard(TimelineEvent event, _TypeConfig cfg, bool isFirst) {
    return Container(
      decoration: BoxDecoration(
        ...
        border: isFirst ? Border.all(color: cfg.color.withOpacity(0.3), width: 1.5) : null,
      ),
        ...
        Container(
          ...
          child: Row(children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: cfg.color.withOpacity(0.12), borderRadius: BorderRadius.circular(8)),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(cfg.icon, size: 9, color: cfg.color),
                const SizedBox(width: 3),
                Text(cfg.label, style: TextStyle(fontSize: 9, color: cfg.color, fontWeight: FontWeight.w700)),
              ])),
            const Spacer(),
            if (isFirst) Container(
              margin: const EdgeInsets.only(right: 6),
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
              decoration: BoxDecoration(color: cfg.color, borderRadius: BorderRadius.circular(6)),
              child: const Text('最新', style: TextStyle(fontSize: 8, color: Colors.white, fontWeight: FontWeight.w700))),
  ...
  Widget _weightBody(TimelineEvent event, _TypeConfig cfg) => Row(
    crossAxisAlignment: CrossAxisAlignment.end, children: [
      Text(event.title, style: TextStyle(
        fontSize: 30, fontWeight: FontWeight.w900, color: cfg.color, height: 1.1)),
      const SizedBox(width: 4),
      const Padding(padding: EdgeInsets.only(bottom: 4),
        child: Text('体重', style: TextStyle(fontSize: 11, color: AppTheme.textHint))),
      if (event.content != null && event.content!.isNotEmpty) ...[
        const SizedBox(width: 8),
        Expanded(child: Text(event.content!,
          style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary), maxLines: 1, overflow: TextOverflow.ellipsis)),
      ],
    ]);

  Widget _medicalBody(TimelineEvent event, _TypeConfig cfg) => Column(
    crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(event.title, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: AppTheme.deepBlue)),
      if (event.content != null && event.content!.isNotEmpty) ...[
        const SizedBox(height: 6),
        ...event.content!.split('\n').map((l) => Padding(
          padding: const EdgeInsets.only(bottom: 3),
          child: Row(children: [
            Container(width: 3, height: 3, margin: const EdgeInsets.only(right: 7, top: 1),
              decoration: BoxDecoration(color: cfg.color.withOpacity(0.5), shape: BoxShape.circle)),
            Expanded(child: Text(l, style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary, height: 1.4))),
          ]))),
      ],
    ]);
  ...
  Widget _buildEmpty() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    Text(_filter == 'all' ? '还没有任何记录' : '暂无${_typeConfig(_filter).label}记录',
      style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppTheme.deepBlue)),
    const SizedBox(height: 8),
    const Text('长按卡片可删除记录', style: TextStyle(fontSize: 12, color: AppTheme.textHint)),
  ]));
  ...
  _TypeConfig _typeConfig(String type) {
    switch (type) {
      case 'growth':  return _TypeConfig(CupertinoIcons.heart_circle_fill, const Color(0xFF4CAF50), '成长点滴');
      case 'medical': return _TypeConfig(CupertinoIcons.heart_fill, AppTheme.danger, '医疗');
      case 'weight':  return _TypeConfig(CupertinoIcons.chart_bar_fill, AppTheme.primaryLight, '体重');
      case 'photo':   return _TypeConfig(CupertinoIcons.photo_fill, const Color(0xFF7E57C2), '照片');
      default:        return _TypeConfig(CupertinoIcons.star_fill, AppTheme.primary, '里程碑'); // 'note' = milestone
    }
  }
}
...
class _CurvePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final path = Path()
      ..moveTo(fromX, 0)
      ..cubicTo(
        fromX, size.height * 0.42,
        toX,   size.height * 0.58,
        toX,   size.height,
      );
  ...
  bool shouldRepaint(_CurvePainter old) =>
    old.progress != progress || old.fromColor != fromColor ||
    old.toColor != toColor || old.goRight != goRight;
}
