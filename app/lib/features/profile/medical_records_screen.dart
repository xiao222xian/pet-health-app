...
class _MedicalRecordsScreenState extends State<MedicalRecordsScreen> {
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() {
      _records = (data as List).map((e) => MedicalRecord.fromJson(e as Map<String, dynamic>)).toList();
      _loading = false;
    });
  }
  ...
  Future<void> _delete(MedicalRecord r) async {
    ...
    if (confirmed == true) {
      await SupabaseService.client.from('medical_records').delete().eq('id', r.id);
      _load();
    }
  }
  ...
  Widget _buildEmpty() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    const Text('还没有医疗记录', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: AppTheme.deepBlue)),
    const SizedBox(height: 8),
    const Text('记录疫苗、驱虫、体检等健康档案', style: TextStyle(fontSize: 13, color: AppTheme.textSecondary)),
  ...
  Widget _buildCard(MedicalRecord r) {
    ...
    final isOverdue = r.nextDueDate != null && r.nextDueDate!.isBefore(DateTime.now());
    final isDueSoon = r.nextDueDate != null && !isOverdue &&
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: (isOverdue || isDueSoon)
            ? Border.all(color: isOverdue ? AppTheme.danger : AppTheme.warning, width: 1.5)
            : null,
        ),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            child: Row(children: [
              Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Text(cfg.label, style: TextStyle(fontSize: 11, color: cfg.color, fontWeight: FontWeight.w700)),
                ])),
              const Spacer(),
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(r.title,
                style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppTheme.deepBlue)),
              if (r.notes != null && r.notes!.isNotEmpty) ...[
                const SizedBox(height: 5),
                Text(r.notes!, style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary, height: 1.5),
              ...
              if (r.nextDueDate != null) ...[
                ...
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(
                      isOverdue ? CupertinoIcons.exclamationmark_circle_fill
                        : CupertinoIcons.clock_fill,
                      size: 12,
                      color: isOverdue ? AppTheme.danger
                        : isDueSoon ? AppTheme.warning
                        : AppTheme.primary),
                    const SizedBox(width: 5),
                    Text(
                      isOverdue ? '已逾期 · 下次 ${_fmtDate(r.nextDueDate!)}'
  ...
  _TypeCfg _typeCfg(String type) {
    switch (type) {
      case 'vaccine':   return _TypeCfg('💉', '疫苗', AppTheme.primary);
      case 'checkup':   return _TypeCfg('🔬', '体检', const Color(0xFF26A69A));
      case 'deworming': return _TypeCfg('💊', '驱虫', const Color(0xFF7E57C2));
      case 'surgery':   return _TypeCfg('🏥', '手术', AppTheme.danger);
      default:          return _TypeCfg('📋', '其他', AppTheme.textSecondary);
    }
  }
}
...
class _RecordFormSheet extends StatefulWidget {
  ...
  const _RecordFormSheet({required this.petId, this.existing, required this.onSaved});
  @override
...
class _RecordFormSheetState extends State<_RecordFormSheet> {
  ...
  @override
  void dispose() {
    _titleCtrl.dispose(); _brandCtrl.dispose(); _dewormTypeCtrl.dispose();
    _notesCtrl.dispose(); _costCtrl.dispose(); _clinicCtrl.dispose();
    super.dispose();
  }
  ...
  String get _titleLabel {
    switch (_type) {
      case 'vaccine':   return '疫苗名称';
      case 'checkup':   return '体检项目';
      case 'deworming': return '驱虫记录';
      case 'surgery':   return '手术名称';
      default:          return '项目名称';
    }
  }
  ...
  String get _nextDueLabel {
    switch (_type) {
      case 'vaccine':   return '下次疫苗时间提醒';
      case 'checkup':   return '下次体检时间提醒';
      case 'deworming': return '下次驱虫时间提醒';
      default:          return '下次时间提醒';
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      final notes = [
        if (_type == 'vaccine' && _brandCtrl.text.trim().isNotEmpty) '疫苗品牌：${_brandCtrl.text.trim()}',
        if (_type == 'deworming' && _dewormTypeCtrl.text.trim().isNotEmpty) '驱虫项目：${_dewormTypeCtrl.text.trim()}',
        if (_type == 'deworming' && _brandCtrl.text.trim().isNotEmpty) '驱虫品牌：${_brandCtrl.text.trim()}',
        if (_clinicCtrl.text.trim().isNotEmpty) '院所：${_clinicCtrl.text.trim()}',
        if (_costCtrl.text.trim().isNotEmpty) '费用：¥${_costCtrl.text.trim()}',
      ...
      final payload = {
        ...
        if (_nextDue != null) 'next_due_date': _nextDue!.toIso8601String().substring(0, 10),
        if (notes.isNotEmpty) 'notes': notes,
      };
      if (widget.existing != null) {
        await SupabaseService.client.from('medical_records').update(payload).eq('id', widget.existing!.id);
      } else {
      ...
      if (mounted) {
        showCupertinoDialog(context: context, builder: (_) => CupertinoAlertDialog(
          title: const Text('保存失败'), content: Text(e.toString()),
  ...
  void _pickDate(bool isNextDue) {
    DateTime tempDate = isNextDue ? (_nextDue ?? DateTime.now().add(const Duration(days: 365))) : _date;
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            CupertinoButton(child: const Text('完成'), onPressed: () {
              setState(() { if (isNextDue) { _nextDue = tempDate; } else { _date = tempDate; } });
              Navigator.pop(context);
            }),
          ]),
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          // Header
          Row(children: [
            Expanded(child: Text(isEdit ? '编辑记录' : '添加医疗记录',
          ...
          SizedBox(
            height: 52,
            child: ListView(scrollDirection: Axis.horizontal, children: _types.map((t) {
              final sel = _type == t.$1;
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: sel ? [BoxShadow(color: color.withOpacity(0.2), blurRadius: 10, offset: const Offset(0,3))] : AppTheme.cardShadow),
                  child: Row(children: [
                    Text(t.$2, style: const TextStyle(fontSize: 16)),
                    const SizedBox(width: 6),
                    Text(t.$3, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700,
          ...
          _label(_titleLabel),
          const SizedBox(height: 8),
          _textField(_titleCtrl, '输入${_titleLabel}', onChanged: (_) => setState(() {})),
          ...
          if (_showNextDue) ...[
            Row(children: [
              ...
              if (_nextDue != null) GestureDetector(
                onTap: () => setState(() => _nextDue = null),
                child: const Text('清除', style: TextStyle(fontSize: 12, color: AppTheme.textHint))),
            ]),
            const SizedBox(height: 8),
            _nextDue != null
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      Container(padding: const EdgeInsets.all(7),
                        ...
                        child: const Icon(CupertinoIcons.calendar_badge_plus, color: AppTheme.primary, size: 16)),
                      const SizedBox(width: 12),
                      Text('设置${_nextDueLabel}', style: const TextStyle(fontSize: 14, color: AppTheme.textHint)),
                      const Spacer(),
                      const Icon(CupertinoIcons.add_circled, color: AppTheme.primary, size: 18),
                    ]),
                  )),
            const SizedBox(height: 16),
          ],
          ...
          Container(
            ...
            child: CupertinoTextField(
              controller: _notesCtrl, placeholder: '例：接种后观察24小时，无异常反应',
              maxLines: 4, minLines: 3, padding: const EdgeInsets.all(14), decoration: null,
  ...
  Widget _textField(TextEditingController ctrl, String hint,
      {TextInputType? keyboardType, String? prefix, ValueChanged<String>? onChanged}) =>
    Container(
      decoration: BoxDecoration(
        color: Colors.white, borderRadius: BorderRadius.circular(14),
        border: Border.all(color: AppTheme.divider), boxShadow: AppTheme.cardShadow),
  ...
  Widget _dateTile(String label, DateTime date, VoidCallback onTap) => GestureDetector(
    ...
    child: Container(
      ...
      child: Row(children: [
        Container(padding: const EdgeInsets.all(7),
          decoration: BoxDecoration(color: AppTheme.primarySoft, borderRadius: BorderRadius.circular(9)),
          child: const Icon(CupertinoIcons.calendar, color: AppTheme.primary, size: 16)),
        const SizedBox(width: 12),
  ...
  Color _typeColor(String t) {
    switch (t) {
      case 'vaccine':   return AppTheme.primary;
      case 'checkup':   return const Color(0xFF26A69A);
      case 'deworming': return const Color(0xFF7E57C2);
      case 'surgery':   return AppTheme.danger;
...
class _RecordDetailSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isOverdue = record.nextDueDate != null && record.nextDueDate!.isBefore(DateTime.now());
    return Container(
      decoration: const BoxDecoration(
        color: AppTheme.background,
        borderRadius: BorderRadius.vertical(top: Radius.circular(28))),
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Row(children: [
            Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                const SizedBox(width: 5),
                Text(cfg.label, style: TextStyle(fontSize: 12, color: cfg.color, fontWeight: FontWeight.w700)),
              ])),
            const Spacer(),
          ...
          // 标题
          Text(record.title,
            style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: AppTheme.deepBlue)),
          const SizedBox(height: 16),
          // 备注
          ...
          if (record.nextDueDate != null) ...[
            Container(
              padding: const EdgeInsets.all(14),
              decoration: BoxDecoration(
                color: isOverdue ? AppTheme.danger.withOpacity(0.07) : AppTheme.primarySoft,
                borderRadius: BorderRadius.circular(14)),
              child: Row(children: [
                Icon(CupertinoIcons.bell_fill,
                  color: isOverdue ? AppTheme.danger : AppTheme.primary, size: 18),
            ...
            Expanded(child: GestureDetector(
              ...
              child: Container(
                ...
                child: const Center(child: Text('删除',
                  style: TextStyle(color: AppTheme.danger, fontWeight: FontWeight.w700, fontSize: 15))),
              ),
            )),
            const SizedBox(width: 12),
            Expanded(flex: 2, child: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(14),
                  boxShadow: AppTheme.cardShadowStrong),
                child: const Center(child: Text('编辑',
                  style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 15))),
              ),
            )),
          ]),
  ...
  Widget _detailRow(IconData icon, String label, String value) => Row(
    ...
    children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(value, style: const TextStyle(fontSize: 14, color: AppTheme.deepBlue, height: 1.5)),
      ])),
    ],
  );
}
