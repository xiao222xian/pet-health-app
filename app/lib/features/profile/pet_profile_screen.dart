...
class _PetProfileScreenState extends State<PetProfileScreen> {
  ...
  Map<String, dynamic>? _stats; // {logs, last_log, weight}
  ...
  Future<void> _load() async {
    ...
    final petsData = await SupabaseService.client
        .from('pets').select().eq('user_id', userId).order('created_at');
    if (!mounted) return;
    final pets = (petsData as List).map((e) => Pet.fromJson(e as Map<String, dynamic>)).toList();
    if (pets.isEmpty) { setState(() { _pets = []; _loading = false; }); return; }
    final idx = _selectedIdx.clamp(0, pets.length - 1);
    await _loadExtras(pets[idx].id);
    if (!mounted) return;
    setState(() { _pets = pets; _selectedIdx = idx; _loading = false; });
  }

  Future<void> _loadExtras(String petId) async {
    final soon = DateTime.now().add(const Duration(days: 30));
    final results = await Future.wait([
      SupabaseService.client.from('medical_records').select()
          .eq('pet_id', petId)
          .lte('next_due_date', soon.toIso8601String().substring(0, 10))
          .order('next_due_date'),
      SupabaseService.client.from('health_logs').select('log_date, weight_kg, appetite_level')
          .eq('pet_id', petId).order('log_date', ascending: false).limit(30),
    ]);
    if (!mounted) return;
    final records = (results[0] as List).map((e) => MedicalRecord.fromJson(e as Map<String, dynamic>)).toList();
  ...
  Widget build(BuildContext context) {
    if (_loading) return const CupertinoPageScaffold(
      backgroundColor: AppTheme.background, child: Center(child: CupertinoActivityIndicator()));
  ...
  Widget _buildEmpty() => CupertinoPageScaffold(
    ...
    child: Stack(children: [
      ...
      SafeArea(child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const Text('还没有添加宠物', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: AppTheme.deepBlue)),
        const SizedBox(height: 8),
        const Text('添加你的毛孩子，开始记录它的生活', style: TextStyle(fontSize: 14, color: AppTheme.textSecondary)),
        const SizedBox(height: 32),
        GestureDetector(
          onTap: () async { await context.push('/pet/new'); await _load(); },
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 36, vertical: 14),
            decoration: BoxDecoration(gradient: AppTheme.primaryGradient,
              borderRadius: BorderRadius.circular(28), boxShadow: AppTheme.cardShadowStrong),
  ...
  Widget _buildProfile(Pet pet) => CupertinoPageScaffold(
    ...
    child: CustomScrollView(slivers: [
      SliverToBoxAdapter(child: Stack(children: [
        Container(height: 320, decoration: const BoxDecoration(
          gradient: LinearGradient(colors: [AppTheme.bgTop, AppTheme.background],
            begin: Alignment.topCenter, end: Alignment.bottomCenter))),
        SafeArea(child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
              Text('我的宠物', style: TextStyle(color: AppTheme.primary.withOpacity(0.7),
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  Expanded(child: Text(pet.name, style: const TextStyle(color: AppTheme.deepBlue,
                    fontSize: 26, fontWeight: FontWeight.w800, letterSpacing: -0.5))),
                  GestureDetector(
                    onTap: () async { await context.push('/pet/edit/${pet.id}'); await _load(); },
                    child: Container(
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        const Icon(CupertinoIcons.pencil, color: AppTheme.primary, size: 12),
                        const SizedBox(width: 4),
                        const Text('编辑', style: TextStyle(color: AppTheme.primary, fontSize: 12, fontWeight: FontWeight.w600)),
                      ]),
                    ),
                  ),
                ]),
                ...
                GestureDetector(
                  ...
                  child: Row(children: [
                    Text(_speciesLabel(pet), style: const TextStyle(color: AppTheme.textSecondary, fontSize: 13)),
                    const SizedBox(width: 4),
                    Icon(
                      _infoExpanded ? CupertinoIcons.chevron_up : CupertinoIcons.chevron_down,
                ...
                if (_infoExpanded) ...[
                  ...
                  Wrap(spacing: 6, runSpacing: 4, children: [
                    if (pet.birthDate != null) _chip(_ageLabel(pet.birthDate!)),
                    if (pet.weightKg != null) _chip('${pet.weightKg}kg'),
                    _chip(pet.gender == 'male' ? '♂ 弟弟' : pet.gender == 'female' ? '♀ 妹妹' : '性别未知'),
      ...
      SliverPadding(
        ...
        sliver: SliverList(delegate: SliverChildListDelegate([
          ...
          _sectionLabel('管理'),
          const SizedBox(height: 10),
          _menuCard(icon: CupertinoIcons.doc_text_fill, color: AppTheme.primary,
            title: '医疗记录', subtitle: '疫苗、驱虫、就诊',
            onTap: () async { await context.push('/medical/${pet.id}'); await _load(); }),
          const SizedBox(height: 10),
          _menuCard(icon: CupertinoIcons.heart_fill, color: const Color(0xFF26A69A),
            title: '健康日志', subtitle: '每日精力、饮食、体重记录',
            onTap: () => context.go('/health')),
          const SizedBox(height: 10),
          _menuCard(icon: CupertinoIcons.time, color: AppTheme.primaryLight,
            title: '生命时光轴', subtitle: '里程碑、成长点滴',
            onTap: () => context.go('/timeline')),
          const SizedBox(height: 20),
          _sectionLabel('其他'),
          const SizedBox(height: 10),
          _menuCard(icon: CupertinoIcons.delete, color: AppTheme.danger,
            title: '删除此宠物档案', subtitle: '删除后数据无法恢复',
        ...
        itemBuilder: (ctx, i) {
          if (i == _pets.length) {
            return GestureDetector(
              onTap: () async { await context.push('/pet/new'); await _load(); },
              child: SizedBox(width: 60, child: Column(
                mainAxisAlignment: MainAxisAlignment.start,
                children: [
                  Container(width: 52, height: 52,
                    margin: const EdgeInsets.only(left: 4),
                    decoration: BoxDecoration(gradient: AppTheme.primaryGradient,
                      shape: BoxShape.circle, boxShadow: AppTheme.cardShadowStrong),
                    child: const Icon(CupertinoIcons.add, color: Colors.white, size: 20)),
                  const SizedBox(height: 5),
                  Text('添加', style: const TextStyle(fontSize: 10, color: AppTheme.primary,
  ...
  Widget _buildStatsRow() {
    ...
    final nextReminder = _upcomingRecords.isNotEmpty ? _upcomingRecords.first : null;
    final reminderLabel = nextReminder != null
        ? _typeShortLabel(nextReminder.type)
        : '-';
    final reminderValue = nextReminder?.nextDueDate != null
        ? '${nextReminder!.nextDueDate!.month}月${nextReminder.nextDueDate!.day}日'
        : (nextReminder != null ? '已添加' : '-');
    ...
    return Row(children: [
      ...
      const SizedBox(width: 8),
      Expanded(child: _statCard('📅', lastLog != null ? _shortDate(lastLog) : '-', '最新记录')),
    ]);
  }

  String _typeShortLabel(String type) {
    switch (type) {
      case 'vaccine': return '疫苗提醒';
      case 'checkup': return '体检提醒';
      case 'deworming': return '驱虫提醒';
      case 'surgery': return '手术记录';
      default: return '备忘提醒';
    }
  }
  ...
  Widget _statCard(String emoji, String value, String label) => Container(
    ...
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      Text(value, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: AppTheme.deepBlue),
        maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center),
  ...
  Widget _chip(String text) => Container(
    ...
    decoration: BoxDecoration(color: AppTheme.primarySoft, borderRadius: BorderRadius.circular(20),
      border: Border.all(color: AppTheme.primary.withOpacity(0.2))),
    child: Text(text, style: const TextStyle(color: AppTheme.primary, fontSize: 11, fontWeight: FontWeight.w600)));
  ...
  Widget _reminderCard(MedicalRecord r) => Container(
    ...
    decoration: BoxDecoration(color: AppTheme.warningSoft, borderRadius: BorderRadius.circular(16),
      border: Border.all(color: AppTheme.warning.withOpacity(0.2))),
    child: Row(children: [
      Container(width: 36, height: 36,
        decoration: BoxDecoration(color: AppTheme.warning.withOpacity(0.15), shape: BoxShape.circle),
        child: const Icon(CupertinoIcons.bell_fill, color: AppTheme.warning, size: 18)),
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(r.title, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14, color: AppTheme.textPrimary)),
      ...
      Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
        ...
        child: const Text('即将到期', style: TextStyle(fontSize: 11, color: AppTheme.warning, fontWeight: FontWeight.w600))),
    ]));
  ...
  String _speciesLabel(Pet pet) {
    final sp = pet.species == 'dog' ? '狗狗' : pet.species == 'cat' ? '猫咪' : '宠物';
